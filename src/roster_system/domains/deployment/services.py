...
from roster_system.schemas import (
    ...
    DeploymentAgentActionResult,
    DeploymentAgentActionType,
...
class DeploymentAgentActionError(Exception):
    pass
...
class DeploymentService:
    ...
    def _find_assignment_by_employee(
        self, assignments: list[DeploymentAssignmentItem], employee_id: int
    ) -> tuple[int, DeploymentAssignmentItem] | tuple[None, None]:
    ...
    def _apply_agent_action(
        self,
        action: DeploymentAgentAction,
        assignments: list[DeploymentAssignmentItem],
    ) -> DeploymentAgentActionResult:
        idx, current = self._find_assignment_by_employee(assignments, action.employee_id)
        if current is None or idx is None:
            raise DeploymentAgentActionError(f"employee_id {action.employee_id} is not assigned on this deployment_date")

        if action.action_type == DeploymentAgentActionType.CANCEL:
            assignments.pop(idx)
            return DeploymentAgentActionResult(
                action_type=action.action_type,
                employee_id=action.employee_id,
                status="APPLIED",
                detail="Assignment cancelled",
                before=current,
                after=None,
            )

        if action.action_type == DeploymentAgentActionType.RETIME:
            if action.from_slot_index != current.slot_index:
                raise DeploymentAgentActionError(
                    f"employee_id {action.employee_id} is currently at slot_index {current.slot_index}, not {action.from_slot_index}"
                )
            updated = DeploymentAssignmentItem(
                site_id=current.site_id,
                slot_index=action.to_slot_index or current.slot_index,
            ...
            return DeploymentAgentActionResult(
                ...
                employee_id=action.employee_id,
                status="APPLIED",
                detail=f"Retimed from slot {current.slot_index} to slot {updated.slot_index}",
                before=current,
                after=updated,
            )

        if action.action_type == DeploymentAgentActionType.CHANGE_GATE:
            target_slot = action.target_slot_index if action.target_slot_index is not None else current.slot_index
            updated = DeploymentAssignmentItem(
                site_id=action.target_site_id or current.site_id,
            ...
            return DeploymentAgentActionResult(
                ...
                employee_id=action.employee_id,
                status="APPLIED",
                detail=f"Moved from site {current.site_id} to site {updated.site_id}",
                before=current,
                after=updated,
            )

        raise DeploymentAgentActionError(f"Unsupported action_type {action.action_type}")

    def run_agent_actions(self, payload: DeploymentAgentRequest) -> DeploymentAgentResponse:
        current = self.list_assignments(payload.deployment_date)
        working = [DeploymentAssignmentItem(**row.model_dump()) for row in current.assignments]
        action_results: list[DeploymentAgentActionResult] = []

        for action in payload.actions:
            result = self._apply_agent_action(action, working)
            action_results.append(result)

        validated_payload = DeploymentAssignmentsUpsert(
            deployment_date=payload.deployment_date,
            assignments=working,
        )

        if payload.auto_apply:
            saved = self.replace_assignments(validated_payload)
            return DeploymentAgentResponse(
                deployment_date=saved.deployment_date,
                dry_run=False,
        ...
        return DeploymentAgentResponse(
            ...
            dry_run=True,
            actions=action_results,
            assignments=validated_payload.assignments,
